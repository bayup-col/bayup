...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const MenuItem = ({ href, label, id }: { href: string, label: any, id: string }) => {
      // Si estamos en la zona Super Admin, mostramos todo lo que se pida
      if (isSuperAdminZone) {
          return (
      ...
      // Lógica original para el dashboard de cliente
        ...
        <aside className="w-full h-full flex flex-col p-4 overflow-y-auto custom-scrollbar">
                      ...
                      <>
                        ...
                        {isSuperAdminZone ? (
                            <button 
                                ...
                                className="w-full py-3 bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                            >
                                <ChevronLeft size={12} /> Volver a mi tienda
                            </button>
                        ) : (
                            <motion.button 
                            ...
                            >
                                <motion.div 
                                    variants={{ hover: { rotate: [0, 360], scale: 1.5 } }}
                                    transition={{ rotate: { duration: 3, repeat: Infinity, ease: "linear" }, scale: { duration: 0.4 } }}
          ...
          <nav className="flex-1 space-y-1">
            {isSuperAdminZone ? (
                ...
                /* MENU TIENDA (PLAN BASICO / PRO / EMPRESA) */
                <>
