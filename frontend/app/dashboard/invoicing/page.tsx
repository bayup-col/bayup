...
export default function InvoicingPage() {
  ...
  // reporte de facturación y lo presente como si fuera el original.
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text('REPORTE DE FACTURACIÓN', 12, 13);
      doc.setFont('helvetica', 'normal'); doc.setFontSize(8);
      doc.text(`Generado ${new Date().toLocaleString('es-CO')} · ${filteredHistory.length} facturas · BAYUP.CO`, 12, 19);

      autoTable(doc, {
        startY: 28,
        head: [['# FACTURA','FECHA','CLIENTE','EMAIL','TELÉFONO','CIUDAD','ORIGEN','MÉTODO PAGO','TOTAL (COP)']],
        body: filteredHistory.map(inv => [
          inv.invoice_num, inv.date, inv.customer,
          inv.customer_email || '—', inv.customer_phone || '—', inv.customer_city || '—',
          inv.source, inv.payment_method, fmtCOP(inv.total),
        ]),
        styles: { font: 'helvetica', fontSize: 8, cellPadding: 2.5 },
        headStyles: { fillColor: [0, 77, 77], textColor: [255, 255, 255], fontStyle: 'bold' },
        alternateRowStyles: { fillColor: [247, 250, 250] },
        columnStyles: { 8: { fontStyle: 'bold', textColor: [0, 77, 77] } },
      });

      doc.save(`facturacion_${new Date().toISOString().slice(0,10)}.pdf`);
