...
export default function InvoicingPage() {
    ...
    const invoicingKpis = useMemo(() => {
        ...
        const highestTicket = history.length > 0 ? Math.max(...history.map(inv => Number(inv.total) || 0)) : 0;
        const lowestTicket = history.length > 0 ? Math.min(...history.map(inv => Number(inv.total) || 0)) : 0;
        ...
        return [
            ...
            { label: 'Flujo de caja', value: totalRevenue, icon: <Wallet size={24}/>, isCurrency: true, color: "text-[#004D4D]", bg: "bg-[#004D4D]/5", trend: "Balance", 
                details: [
                    { l: "BRUTO", v: `$ ${totalRevenue.toLocaleString()}`, icon: <DollarSign size={10}/> },
                    { l: "NETO (EST)", v: `$ ${(totalRevenue * 0.7).toLocaleString()}`, icon: <ShieldCheck size={10}/> },
                    { l: "CRECIMIENTO", v: "+5.2%", icon: <TrendingUp size={10}/> }
                ], 
                advice: "Tu flujo de caja es saludable. Te sugiero reinvertir un 10% en publicidad web para escalar tus ventas digitales." 
            }
        ];
    }, [history]);
            ...
            <AnimatePresence mode="wait">
                {!isPOSActive ? (
                    <div className="space-y-10 p-4">
                        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 px-4 overflow-visible">
            ...
            <AnimatePresence>
                {selectedProductForVariant && (
                    <div className="fixed inset-0 z-[4000] flex items-center justify-center">
                        {/* Backdrop de Inmersión Total */}
                        <motion.div 
                            initial={{ opacity: 0 }} 
                            animate={{ opacity: 1 }} 
                            exit={{ opacity: 0 }} 
                            onClick={() => setSelectedProductForVariant(null)} 
                            className="absolute inset-0 bg-[#001A1A]/98 backdrop-blur-3xl" 
                        />
                        ...
                        <motion.div 
                            ...
                            className="relative bg-white w-full h-full flex flex-col md:flex-row text-slate-900 overflow-hidden"
                        >
                            {/* Panel Izquierdo: Galería de Activos */}
                            <div className="w-full md:w-[50%] bg-[#F8FAFC] flex flex-col p-12 lg:p-20 space-y-8 shrink-0 relative">
                                <div className="flex-1 rounded-[3.5rem] overflow-hidden bg-white shadow-2xl flex items-center justify-center relative group border border-gray-100">
                                    {(() => {
                                        ...
                                        return mainImg ? <img src={mainImg} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt="Main"/> : <Package size={120} className="text-gray-100" />;
                                    })()}
                                    <div className="absolute top-10 left-10 px-6 py-2 bg-white/90 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-[0.2em] text-[#004D4D] shadow-xl">Activo Verificado</div>
                                </div>
                                <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 px-2">
                                    {Array.isArray(selectedProductForVariant.image_url) && selectedProductForVariant.image_url.map((url: string, i: number) => (
                                        <button key={i} onClick={() => setCurrentImageIndex(i)} className={`h-24 w-24 rounded-[2rem] overflow-hidden flex-shrink-0 border-4 transition-all ${currentImageIndex === i ? 'border-[#004D4D] scale-90 shadow-lg' : 'border-white opacity-40 hover:opacity-100'}`}><img src={url} className="w-full h-full object-cover" /></button>
                                    ))}
                                </div>
                            </div>

                            {/* Panel Derecho: Configuración de Venta */}
                            <div className="flex-1 p-12 lg:p-20 overflow-y-auto custom-scrollbar flex flex-col justify-between space-y-12 bg-white">
                                <div className="space-y-10">
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-4">
                                            <span className="px-4 py-1.5 bg-[#004D4D] text-white rounded-full text-[9px] font-black uppercase tracking-[0.2em]">{selectedProductForVariant.category}</span>
                                            <div className="h-px flex-1 bg-gray-100" />
                                        </div>
                                        <h3 className="text-5xl lg:text-6xl font-black italic uppercase tracking-tighter text-[#001A1A] leading-[0.9]">{selectedProductForVariant.name}</h3>
                                    </div>

                                    <div className="p-10 bg-gray-50 rounded-[3rem] border border-gray-100 relative isolate">
                                        <div className="absolute top-0 right-10 -translate-y-1/2 bg-white px-4 py-1 rounded-full border border-gray-100 text-[8px] font-black text-gray-300 uppercase tracking-widest">Descripción del Activo</div>
                                        <p className="text-sm text-gray-500 font-medium leading-relaxed italic">"{selectedProductForVariant.description || 'Este activo no cuenta con una descripción técnica detallada en el inventario actual.'}"</p>
                                    </div>

                                    {/* Selectores de Variantes con UI Platinum */}
                                    <div className="space-y-10">
                                        {selectedProductForVariant.variants && selectedProductForVariant.variants.length > 0 ? (
                                            Array.from(new Set(selectedProductForVariant.variants?.map(v => v.name))).map(attrName => (
                                                <div key={attrName} className="space-y-5">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <label className="text-[11px] font-black text-[#004D4D] uppercase tracking-[0.3em]">{attrName}</label>
                                                    </div>
                                                    <div className="flex flex-wrap gap-3">
                                                        {selectedProductForVariant.variants?.filter(v => v.name === attrName).map(v => (
                                                            <button 
                                                                ...
                                                                disabled={v.stock <= 0} 
                                                                className={`px-8 py-5 rounded-[1.8rem] text-[11px] font-black uppercase border-2 transition-all duration-500 ${selectedVariants[attrName]?.id === v.id ? 'bg-[#001A1A] border-[#001A1A] text-white shadow-[0_20px_40px_rgba(0,0,0,0.2)] scale-105' : 'bg-white border-gray-100 text-gray-400 hover:border-[#004D4D]/30'}`}
                                                            >
                                                                {v.sku}
                                                            </button>
                                                        ))}
                                            ...
                                            <div className="w-full flex flex-col p-12 bg-gray-50 rounded-[3.5rem] border-2 border-dashed border-gray-200 space-y-8 items-center text-center">
                                                <div className="h-16 w-16 bg-white rounded-2xl flex items-center justify-center shadow-sm text-gray-200"><Layers size={32}/></div>
                                                <div className="space-y-2"><p className="text-xs font-black text-gray-900 uppercase">Configuración de variante rápida</p><p className="text-[10px] text-gray-400 max-w-[200px]">Define atributos temporales para este pedido específico.</p></div>
                                                <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
                                                    <input value={quickVariant.name} onChange={e => setQuickVariant({...quickVariant, name: e.target.value})} placeholder="Ej: Talla" className="w-full p-4 bg-white border border-gray-100 rounded-2xl outline-none text-xs font-bold" />
                                                    <input value={quickVariant.sku} onChange={e => setQuickVariant({...quickVariant, sku: e.target.value})} placeholder="Ej: XL" className="w-full p-4 bg-white border border-gray-100 rounded-2xl outline-none text-xs font-bold" />
                                                </div>
                                                <button onClick={handleCreateQuickVariant} disabled={!quickVariant.name || !quickVariant.sku || isCreatingQuickVariant} className="w-full max-w-sm py-4 bg-[#004D4D] text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-black transition-all">{isCreatingQuickVariant ? 'Procesando...' : 'Vincular Variante'}</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-8 pt-12 border-t border-gray-100">
                                    <div className="flex flex-col md:flex-row gap-8 items-center">
                                        <div className="flex-1 w-full space-y-3">
                                            <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.3em] ml-2">Monto Final de Transacción</label>
                                            <div className="relative group">
                                                <span className="absolute left-10 top-1/2 -translate-y-1/2 text-[#004D4D] font-black text-3xl opacity-40 group-focus-within:opacity-100 transition-opacity">$</span>
                                                <input 
                                                    type="text" 
                                                    value={tempPrice.toLocaleString('de-DE')} 
                                                    onChange={(e) => setTempPrice(Number(e.target.value.replace(/\D/g, '')))} 
                                                    className="w-full pl-20 pr-10 py-8 bg-[#004D4D]/5 border-2 border-transparent focus:border-[#004D4D]/20 focus:bg-white rounded-[2.5rem] outline-none text-4xl font-black text-[#004D4D] shadow-inner transition-all" 
                                                />
                                            </div>
                                        </div>
                                        <button 
                                            onClick={() => addToCart(selectedProductForVariant, selectedVariants, tempPrice)} 
                                            className="w-full md:w-auto px-16 h-[100px] bg-[#001A1A] text-white rounded-[2.5rem] font-black text-[12px] uppercase tracking-[0.4em] shadow-[0_30px_60px_rgba(0,0,0,0.3)] hover:bg-black hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4"
                                        >
                                            <ShoppingCart size={24}/> 
                                            Sumar a Factura
                                        </button>
                                    </div>
                                    <button onClick={() => setSelectedProductForVariant(null)} className="w-full text-[10px] font-black uppercase text-gray-300 hover:text-rose-500 transition-all tracking-[0.5em] text-center">Descartar Operación y Volver</button>
                                </div>
                            </div>

                            {/* BOTÓN CERRAR FLOTANTE */}
                            <button onClick={() => setSelectedProductForVariant(null)} className="absolute top-10 right-10 h-14 w-14 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition-all z-50">
