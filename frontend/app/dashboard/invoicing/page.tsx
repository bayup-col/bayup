...
export default function InvoicingPage() {
    ...
    const handleFinalize = async () => {
        ...
        } catch (e) { showToast("Error", "error"); } finally { setIsProcessing(false); }
    };

    const handleExportInvoicesPDF = async () => {
        if (filteredHistory.length === 0) { showToast("Sin facturas", "info"); return; }
        try {
            const { generateInvoicesAuditPDF } = await import('@/lib/report-generator');
            await generateInvoicesAuditPDF({ userName: authEmail?.split('@')[0] || 'Empresario', invoices: filteredHistory, range: dateRange });
    ...
    const invoicingKpis = useMemo(() => {
        ...
        return [
            ...
            { 
                ...
                trend: "Total", 
                details: [
                    { l: "Canal POS", v: `${history.filter(o => o.source === 'pos').length}`, icon: <Store size={14}/> },
                    { l: "Canal WhatsApp", v: `${history.filter(o => o.source === 'WhatsApp').length}`, icon: <MessageSquare size={14}/> },
                    { l: "Otros Canales", v: `${history.filter(o => o.source !== 'pos' && o.source !== 'WhatsApp').length}`, icon: <Globe size={14}/> }
                ], 
                advice: "Diversifica tus canales de venta para aumentar la visibilidad de tu marca." 
            },
            { 
                ...
                trend: "Market", 
                details: [
                    { l: "Venta Máxima", v: `$ ${history.length ? Math.max(...history.map(o => o.total)).toLocaleString() : 0}`, icon: <TrendingUp size={14}/> },
                    { l: "Venta Mínima", v: `$ ${history.length ? Math.min(...history.map(o => o.total)).toLocaleString() : 0}`, icon: <ArrowDownRight size={14}/> }
                ], 
                advice: "Crea 'bundles' o combos de productos relacionados para elevar el valor de cada ticket." 
            },
            { 
                label: 'Flujo Caja', 
                value: totalRevenue, 
                ...
                trend: "Balance", 
                details: [
                    { l: "Ingresos Brutos", v: `$ ${totalRevenue.toLocaleString()}`, icon: <Plus size={14}/> },
                    { l: "Comisiones (2.5%)", v: `- $ ${(totalRevenue * 0.025).toLocaleString()}`, icon: <ShieldCheck size={14}/> }
                ], 
                advice: "Este es tu flujo bruto. Recuerda registrar tus gastos para ver la utilidad neta real." 
            }
        ];
    }, [history]);
                            ...
                            <div className="flex flex-col md:flex-row items-center gap-4 w-full max-w-6xl mx-auto">
                                ...
                                <div className="flex items-center gap-3">
                                    <motion.button 
                                        ...
                                        className={`h-16 flex items-center gap-2 px-6 rounded-3xl border transition-all ${isHistoryFilterOpen ? 'bg-[#004D4D] text-white border-[#004D4D]' : 'bg-white border-white/80 text-gray-500 shadow-sm'}`}
                                    >
                                        <Filter size={20}/> 
                                        <AnimatePresence>
                                            {isFilterHovered && (
                                                <motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap overflow-hidden">Filtros</motion.span>
                                            )}
                                        </AnimatePresence>
                                    </motion.button>
                                    ...
                                    <motion.button 
                                        ...
                                        onClick={() => setIsHistoryFilterOpen(!isHistoryFilterOpen)}
                                        className="h-16 flex items-center gap-2 px-6 rounded-3xl bg-white border border-white/80 text-gray-500 hover:text-[#004D4D] shadow-sm transition-all"
                                    >
                                        <Calendar size={20}/>
                                        <AnimatePresence>
                                            {isDateHovered && (
                                ...
                                <table className="w-full text-center">
                                    ...
                                    <tbody className="divide-y divide-gray-100/50">
                                        {filteredHistory.map((inv) => (
                                            <tr key={inv.id} onClick={() => handleViewDetail(inv)} className="hover:bg-white/60 transition-all cursor-pointer group active:bg-gray-100/50">
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[3000] flex flex-col lg:flex-row overflow-hidden bg-white">
                        <div className="flex-1 flex flex-col bg-[#FAFAFA] border-r border-gray-100 overflow-hidden relative">
                            <header className="p-8 border-b bg-white flex justify-between items-center shrink-0">
                                <button onClick={() => setIsPOSActive(false)} className="flex items-center gap-2 text-[10px] font-black uppercase text-gray-400 hover:text-rose-500 transition-all"><ArrowLeft size={16}/> Cancelar</button>
                            </header>
                            <div className="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar">
                                <div className="space-y-8">
                                    <div className="flex p-1 bg-gray-100 rounded-2xl w-fit">
                                        <button onClick={() => setPosCustomerMode('create')} className={`px-8 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${posCustomerMode === 'create' ? 'bg-white text-[#004D4D] shadow-md' : 'text-gray-400'}`}>Registrar</button>
                                        <button onClick={() => setPosCustomerMode('search')} className={`px-8 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${posCustomerMode === 'search' ? 'bg-white text-[#004D4D] shadow-md' : 'text-gray-400'}`}>Buscar</button>
                                    </div>
                                    {posCustomerMode === 'create' && (
                                        <div className="grid grid-cols-2 gap-6 bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm animate-in slide-in-from-top-4">
                                            ...
                                            <div className="space-y-2"><label className="text-[9px] font-black text-gray-400 uppercase ml-1">Ciudad</label><input value={customerInfo.city} onChange={e => setCustomerInfo({...customerInfo, city: e.target.value})} className="w-full p-4 bg-gray-50 border-transparent rounded-2xl outline-none font-bold text-sm focus:bg-white focus:border-[#004D4D]/10" placeholder="Ciudad" /></div>
                                            <div className="space-y-2 relative"><label className="text-[9px] font-black text-gray-400 uppercase ml-1">Canal</label><button onClick={() => setIsSourceDropdownOpen(!isSourceDropdownOpen)} className="w-full p-4 bg-gray-50 rounded-2xl flex items-center justify-between font-bold text-sm hover:bg-white"><span className="text-[#004D4D]">{customerInfo.source}</span><ChevronDown size={16}/></button>
                                            ...
                                            <div className="space-y-2"><label className="text-[9px] font-black text-gray-400 uppercase ml-1">Tarifa:</label><div className="flex bg-gray-50 p-1 rounded-2xl border border-gray-100 h-[54px] relative isolate"><button onClick={() => setCustomerInfo({...customerInfo, type: 'final'})} className={`flex-1 rounded-xl text-[9px] font-black uppercase z-10 ${customerInfo.type === 'final' ? 'text-white' : 'text-gray-400'}`}>Final</button><button onClick={() => setCustomerInfo({...customerInfo, type: 'mayorista'})} className={`flex-1 rounded-xl text-[9px] font-black uppercase z-10 ${customerInfo.type === 'mayorista' ? 'text-white' : 'text-gray-400'}`}>Mayorista</button><motion.div animate={{ x: customerInfo.type === 'final' ? 0 : '100%' }} className="absolute inset-y-1 left-1 w-[calc(50%-4px)] bg-[#004D4D] rounded-xl shadow-lg -z-0" /></div></div>
                                        </div>
                                    )}
                                </div>
                                <div className="space-y-8">
                                    <div className="space-y-4"><label className="text-[9px] font-black text-gray-400 uppercase ml-1">Medio de Pago:</label><div className="grid grid-cols-2 gap-4"><button onClick={() => setPaymentMethod('cash')} className={`py-4 rounded-2xl font-black text-[9px] uppercase tracking-widest border-2 transition-all ${paymentMethod === 'cash' ? 'bg-[#004D4D] border-[#004D4D] text-white' : 'bg-white border-gray-100 text-gray-400'}`}>Efectivo</button><button onClick={() => setPaymentMethod('transfer')} className={`py-4 rounded-2xl font-black text-[9px] uppercase tracking-widest border-2 transition-all ${paymentMethod === 'transfer' ? 'bg-[#004D4D] border-[#004D4D] text-white' : 'bg-white border-gray-100 text-gray-400'}`}>Transferencia</button></div></div>
                                    ...
                                    <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">{categories.map(cat => <button key={cat} onClick={() => setSelectedCategory(cat)} className={`flex-shrink-0 px-8 py-3 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${selectedCategory === cat ? 'bg-[#004D4D] text-white shadow-lg' : 'bg-white border text-gray-400'}`}>{cat}</button>)}</div>
                        ...
                        <div className="flex-1 bg-[#F3F4F6] p-8 lg:p-16 flex items-center justify-center relative">
                            ...
                            <div className="w-full max-w-2xl bg-white shadow-[0_40px_80px_-15px_rgba(0,0,0,0.1)] rounded-[4rem] flex flex-col h-full overflow-hidden relative border border-white animate-in zoom-in-95 duration-700 z-10">
                                <div className="bg-white p-12 flex justify-between items-center shrink-0 border-b border-gray-50"><div className="space-y-1"><div className="flex items-center gap-2 mb-3"><div className="h-1.5 w-1.5 rounded-full bg-[#00F2FF] animate-ping"/><span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#004D4D]">Documento Oficial de Venta</span></div>{companyData?.logo_url ? <img src={companyData.logo_url} className="h-12 w-auto object-contain" /> : <h2 className="text-3xl font-black italic tracking-tighter uppercase leading-none text-[#001A1A]">{companyData?.full_name || authEmail?.split('@')[0]}</h2>}</div><div className="text-right"><p className="text-[9px] font-black text-gray-300 uppercase">Comprobante</p><p className="text-lg font-black text-[#004D4D]">#{String(history.length + 1).padStart(4, '0')}</p></div></div>
                                <div className="flex-1 overflow-y-auto custom-scrollbar bg-white p-12 space-y-12">
                                    <div className="grid grid-cols-2 gap-12 text-slate-900">
                                        <div className="space-y-4"><p className="text-[8px] font-black text-gray-300 uppercase tracking-[0.2em] bg-gray-50 w-fit px-3 py-1 rounded-full">Datos del Cliente</p><h4 className="text-xl font-black italic uppercase text-gray-900 leading-tight">{customerInfo.name || 'Cliente Particular'}</h4><div className="flex flex-col gap-2"><div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase"><Mail size={12} className="text-[#004D4D]"/> {customerInfo.email || 'Sin correo'}</div><div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase"><Smartphone size={12} className="text-[#004D4D]"/> {customerInfo.phone || 'Sin número'}</div><div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase"><MapPin size={12} className="text-[#004D4D]"/> {customerInfo.city || 'Sin ciudad'}</div></div></div>
                                        <div className="text-right space-y-4"><p className="text-[8px] font-black text-gray-300 uppercase tracking-[0.2em] bg-gray-50 w-fit px-3 py-1 rounded-full ml-auto">Datos del Vendedor</p><h4 className="text-sm font-black text-[#004D4D] uppercase">{companyData?.full_name}</h4><div className="flex flex-col items-end gap-2"><div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase justify-end">{companyData?.email || authEmail} <Mail size={12} className="text-[#004D4D]"/></div><div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase justify-end">{companyData?.phone || 'Sin WhatsApp'} <Smartphone size={12} className="text-[#004D4D]"/></div><div className="flex items-center gap-2 text-[9px] font-black text-emerald-500 uppercase mt-2"><ShieldCheck size={12}/> Operación Verificada</div></div></div>
                                    </div>
                                    <div className="space-y-6"><div className="flex items-center text-[8px] font-black text-gray-300 uppercase tracking-[0.3em] px-4"><span className="flex-1">Descripción del Activo</span><span className="w-16 text-center">Qty</span><span className="w-28 text-right">Subtotal</span></div><div className="space-y-2">{invoiceItems.map((item, i) => (
                                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={i} className="flex items-center px-6 py-5 bg-gray-50/50 rounded-3xl group"><div className="flex-1 min-w-0"><p className="text-xs font-black text-gray-900 uppercase truncate">{item.name}</p><p className="text-[7px] font-bold text-[#004D4D] uppercase">Ref: {item.sku}</p></div><div className="w-16 flex items-center justify-center gap-2"><button onClick={() => {const ni = [...invoiceItems]; if(ni[i].quantity > 1) ni[i].quantity--; setInvoiceItems(ni);}} className="h-5 w-5 bg-white border rounded">-</button><span className="text-xs font-black text-gray-900">{item.quantity}</span><button onClick={() => {const ni = [...invoiceItems]; ni[i].quantity++; setInvoiceItems(ni);}} className="h-5 w-5 bg-white border rounded">+</button></div><div className="w-28 text-right font-black text-xs text-[#004D4D]">${(item.price * item.quantity).toLocaleString()}</div><button onClick={() => setInvoiceItems(invoiceItems.filter((_, idx) => idx !== i))} className="ml-4 text-rose-200 hover:text-rose-500"><Trash2 size={14}/></button></motion.div>))}</div></div>
                                    <div className="pt-10 mt-auto"><div className="bg-gray-900 p-10 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden group border border-white/5"><div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center"><div className="space-y-4"><div className="flex items-center gap-3"><div className="h-1.5 w-1.5 rounded-full bg-[#00F2FF]"/><p className="text-[10px] font-black uppercase text-white/40 tracking-widest">Resumen de Liquidación</p></div><p className="text-[10px] font-medium text-white/30 italic max-w-xs">Comprobante oficial de compra.</p></div><div className="space-y-6 flex flex-col items-center justify-center text-center px-2"><p className="text-[8px] font-black uppercase text-white/40 mb-2 tracking-[0.2em]">Total Neto a Pagar</p><h3 className="text-4xl font-black tracking-tighter text-white truncate"><AnimatedNumber value={calculateSubtotal()} /></h3></div></div></div></div>
                                </div>
                                <div className="p-10 bg-gray-50 border-t flex gap-4 shrink-0"><button type="button" onClick={() => showToast("Descargando...", "info")} className="flex-1 h-16 bg-white border border-gray-200 text-gray-400 rounded-2xl font-black text-[10px] uppercase hover:text-[#004D4D] transition-all flex items-center justify-center gap-3 shadow-sm"><Download size={18}/> Descargar</button><button onClick={handleFinalize} disabled={isProcessing || invoiceItems.length === 0} className="flex-[2] h-16 bg-[#001A1A] text-white rounded-2xl font-black text-[11px] uppercase shadow-2xl hover:bg-black transition-all flex items-center justify-center gap-3 disabled:opacity-30">{isProcessing ? <Loader2 className="animate-spin" size={18}/> : <CheckCircle2 size={18}/>} FACTURAR</button></div>
                            </div>
                        </div>

                        {/* MODAL DETALLE DE FACTURA (HISTORIAL) */}
                        <AnimatePresence>
                            {selectedInvoice && (
