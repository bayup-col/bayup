                ...
                <table className="min-w-full divide-y divide-gray-100">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {filteredCompanies.map((company) => (
                            <tr key={company.id} onClick={() => setSelectedCompany(company)} className="hover:bg-purple-50/30 transition-colors cursor-pointer group">
                                <td className="px-8 py-5">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div><p className="text-sm font-black text-gray-900">{company.company_name}</p><p className="text-[10px] font-bold text-purple-600 uppercase tracking-tighter">{company.niche}</p></div>
                                    </div>
                                </td>
                                <td className="px-8 py-5"><p className="text-sm font-medium text-gray-700">{company.owner_name}</p></td>
                                <td className="px-8 py-5"><span className={`text-[10px] font-black uppercase px-3 py-1 rounded-lg ${company.plan === 'Gold' ? 'bg-amber-100 text-amber-700' : 'bg-purple-100 text-purple-700'}`}>{company.plan}</span></td>
                                ...
                                <td className="px-8 py-5 text-right"><span className="text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity font-black text-xs uppercase tracking-widest">Detalles →</span></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl border border-gray-100 overflow-hidden flex flex-col animate-in zoom-in-95 duration-300 max-h-[90vh]">
                        <div className="p-10 border-b border-gray-50 relative bg-gradient-to-r from-gray-50 to-white">
                            ...
                            <div className="flex items-center gap-6">
                                <div className="h-24 w-24 rounded-[2rem] bg-gradient-to-tr from-purple-600 to-indigo-600 shadow-xl flex items-center justify-center text-3xl font-black text-white">{selectedCompany.avatar}</div>
                        ...
                        <div className="p-10 overflow-y-auto custom-scrollbar space-y-8">
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div className="bg-purple-50 p-6 rounded-3xl border border-purple-100 text-center"><p className="text-[10px] font-black text-purple-400 uppercase tracking-widest mb-2">Nuestra Ganancia</p><p className="text-2xl font-black text-purple-700">{formatCurrency(selectedCompany.our_profit)}</p></div>
                            </div>
                        </div>
                    </div>
                </div>
