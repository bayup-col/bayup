                        ...
                        <motion.div key="variants" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-10">
                            <div className="p-10 bg-white rounded-[3rem] border border-gray-100 shadow-sm space-y-10">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-sm font-black text-[#004D4D] uppercase tracking-widest flex items-center gap-3">
                                        <Layers size={18} /> Inventario por Atributo
                                    </h3>
                                ...
                                {/* CABECERA DE TABLA */}
                                <div className="grid grid-cols-12 gap-6 px-6 pb-4 border-b border-gray-50">
                                    <div className="col-span-4 text-[9px] font-black text-gray-400 uppercase tracking-widest">Atributo (Ej: Talla)</div>
                                    <div className="col-span-4 text-[9px] font-black text-gray-400 uppercase tracking-widest">Especificación (Ej: XL)</div>
                                    <div className="col-span-3 text-center text-[9px] font-black text-gray-400 uppercase tracking-widest">Stock Disponible</div>
                                ...
                                <div className="space-y-4">
                                    {variants.map((v, idx) => (
                                        <div key={v.id || idx} className="grid grid-cols-12 gap-6 items-center p-4 bg-gray-50/50 rounded-[2rem] hover:bg-white hover:shadow-xl transition-all group">
                                            <div className="col-span-4">
                                                <input 
                                                    placeholder="Atributo..."
                                                    value={v.name}
                                                    onChange={(e) => setVariants(prev => prev.map(item => item.id === v.id ? { ...item, name: e.target.value } : item))}
                                                    className="w-full bg-white rounded-xl px-5 py-4 text-xs font-bold text-[#004D4D] outline-none shadow-sm border border-transparent focus:border-[#00F2FF]/30"
                                                />
                                            </div>
                                            <div className="col-span-4">
                                                <input 
                                                    placeholder="Valor..."
                                                    value={v.sku}
                                                    onChange={(e) => setVariants(prev => prev.map(item => item.id === v.id ? { ...item, sku: e.target.value } : item))}
                                                    className="w-full bg-white rounded-xl px-5 py-4 text-xs font-black text-gray-600 outline-none shadow-sm border border-transparent focus:border-[#00F2FF]/30 uppercase"
                                                />
                                            </div>
                                            <div className="col-span-3">
                                                <input 
                                                    type="number"
                                                    value={v.stock}
                                                    onChange={(e) => setVariants(prev => prev.map(item => item.id === v.id ? { ...item, stock: Number(e.target.value) } : item))}
                                                    className="w-full bg-[#004D4D]/5 rounded-xl px-4 py-4 text-center text-xs font-black text-[#004D4D] outline-none border border-transparent focus:bg-white"
                                                />
                                            </div>
                                ...
                                >
                                    <Plus size={16} /> Añadir Atributo / Variante
                                </button>
                            </div>

                            {/* RESUMEN DE INVENTARIO CONSOLIDADO (DANIEL'S PICK) */}
                            <div className="p-10 bg-[#004D4D] rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
                                    ...
                                    <div className="bg-black/20 backdrop-blur-md px-12 py-6 rounded-[2.5rem] border border-white/10 text-center min-w-[200px]">
