...
export default function CustomersPage() {
    ...
    return customers.filter(c => {
        ...
        if (filterDateRange !== 'all') {
            ...
            if (filterDateRange === 'today') {
                ...
                if (joinDate.getMonth() !== now.getMonth() || joinDate.getFullYear() !== now.getFullYear()) return false;
            }
        }

        return true;
    });
  }, [customers, searchTerm, activeTab, filterStatus, filterDateRange]);
        ...
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 100 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 100 }} className="bg-white w-full max-w-5xl h-[80vh] rounded-[4rem] shadow-3xl overflow-hidden flex flex-col relative z-10 border border-white/20">
                <div className="bg-gray-900 p-10 text-white shrink-0 relative overflow-hidden">
                    ...
                    <button onClick={() => setIsCreatingCustomer(false)} className="absolute top-10 right-10 h-12 w-12 bg-white/5 rounded-2xl flex items-center justify-center hover:bg-rose-50 transition-all z-20"><X size={24} /></button>
                    <div className="relative z-10 flex items-center gap-8">
                        ...
                        <div>
                            <h3 className="text-3xl font-black uppercase italic tracking-tighter">Registrar <span className="text-[#00f2ff]">Nuevo Cliente</span></h3>
                            <div className="flex items-center gap-2 mt-3">
                                {[1, 2].map(s => <div key={s} className={`h-1.5 rounded-full transition-all duration-500 ${wizardStep >= s ? 'w-12 bg-[#00f2ff]' : 'w-4 bg-white/10'}`} />)}
                                <span className="ml-4 text-[10px] font-black uppercase text-[#00f2ff]/60 tracking-widest">{wizardStep === 1 ? 'Identidad' : 'Confirmación'}</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        {wizardStep === 1 && (
                            <motion.div key="s1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-10">
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Email Principal</label><input type="email" value={newCustomer.email} onChange={(e) => setNewCustomer({...newCustomer, email: e.target.value})} className="w-full p-6 bg-white border-2 border-gray-100 rounded-3xl outline-none focus:border-[#00f2ff] font-bold shadow-inner" placeholder="ana@email.com" /></div>
                                    <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Teléfono (WhatsApp)</label><input type="text" value={newCustomer.phone} onChange={(e) => setNewCustomer({...newCustomer, phone: e.target.value})} className="w-full p-6 bg-white border-2 border-gray-100 rounded-3xl outline-none focus:border-[#00f2ff] font-bold shadow-inner" placeholder="+57 300..." /></div>
                                </div>
                                <div className="flex justify-center pt-4"><button disabled={!newCustomer.name || !newCustomer.email} onClick={() => setWizardStep(2)} className="px-16 py-5 bg-gray-900 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-[0.3em] shadow-xl hover:scale-105 transition-all">Siguiente: Validar Perfil</button></div>
                            </motion.div>
                        )}
                        {wizardStep === 2 && (
                            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="max-w-2xl mx-auto space-y-8 text-center">
                                <div className="h-24 w-24 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 animate-bounce"><CheckCircle2 size={48}/></div>
                                <h4 className="text-3xl font-black italic uppercase">Perfil Pre-Validado</h4>
                                <p className="text-gray-500 font-medium italic">"El sistema Bayup ha verificado que el email **{newCustomer.email}** está disponible. El cliente será registrado en la Cartera General."</p>
                                <div className="bg-gray-900 p-10 rounded-[3rem] text-white flex items-center justify-between gap-10 mt-10">
                                    <div className="text-left"><span className="text-[10px] font-black uppercase text-[#00f2ff] tracking-[0.3em]">IA Registration</span><h5 className="text-2xl font-black italic uppercase mt-2">¿Confirmar Alta?</h5></div>
                                    <button onClick={handleCreate} className="px-12 py-6 bg-[#00f2ff] text-gray-900 rounded-[2rem] font-black uppercase text-xs tracking-[0.4em] shadow-3xl hover:bg-white transition-all active:scale-95">Registrar Ahora</button>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                ...
                <div className="p-8 bg-white border-t border-gray-100 flex justify-between items-center px-16">
                    ...
                    <button onClick={() => wizardStep < 2 && setWizardStep(wizardStep + 1)} className={`text-[10px] font-black uppercase tracking-widest text-[#004d4d] ${wizardStep === 2 ? 'opacity-0' : ''}`}>Siguiente <ArrowRight size={14}/></button>
                </div>
            </motion.div>
        </div>
    );
  ...
  const renderCustomerDetail = () => {
    ...
    return (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedCustomer(null)} className="absolute inset-0 bg-black/90 backdrop-blur-xl" />
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 100 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 100 }} className="bg-[#FAFAFA] w-full max-w-4xl rounded-[4rem] shadow-3xl overflow-hidden flex flex-col border border-white/20">
                ...
                <div className="p-12 space-y-10 overflow-y-auto max-h-[60vh] custom-scrollbar">
                    ...
                    <div className="pt-6 border-t border-gray-100 flex gap-4">
                        <button onClick={() => { 
                            window.open(`https://wa.me/${selectedCustomer.phone?.replace(/\D/g, '')}`, '_blank');
                            showToast("Abriendo WhatsApp Business...", "info");
                        }} className="flex-1 py-5 bg-emerald-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-xl hover:bg-emerald-600 transition-all flex items-center justify-center gap-3"><MessageCircle size={16}/> Contactar por WhatsApp</button>
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8 px-4 shrink-0">
            <div>
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#004d4d]/60">Customer Intelligence System</span>
                </div>
            ...
            <div className="flex items-center gap-4">
                <button onClick={() => { setIsCreatingCustomer(true); setWizardStep(1); }} className="h-14 px-10 bg-gray-900 text-white rounded-full font-black text-[10px] uppercase tracking-widest shadow-2xl hover:scale-105 transition-all flex items-center gap-4 group">
                    <Plus size={18} className="text-[#00f2ff] group-hover:rotate-90 transition-transform duration-500" /> Nuevo Cliente
                </button>
            </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-4 shrink-0 relative z-20">
            <div className="p-1.5 bg-white border border-gray-100 rounded-full shadow-xl shadow-gray-200/50 backdrop-blur-xl flex items-center relative">
                {[ { id: 'todos', label: 'Todos', icon: <LayoutGrid size={14}/> }, { id: 'nuevos', label: 'Nuevos', icon: <Zap size={14}/> }, { id: 'vip', label: 'Segmento VIP', icon: <Star size={14}/> }, { id: 'tienda', label: 'POS Físico', icon: <ShoppingBag size={14}/> } ].map((tab) => {
                    ...
                    return (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`relative px-8 py-3.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all duration-500 z-10 flex items-center gap-2 ${isActive ? 'text-white' : 'text-gray-400 hover:text-gray-900'}`}>{isActive && ( <motion.div layoutId="activeCustTab" className="absolute inset-0 bg-[#004D4D] rounded-full shadow-lg -z-10" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} /> )}{tab.icon}{tab.label}</button>
                    );
                })}
            </div>
                    ...
                    <motion.button layout onMouseEnter={() => setIsFilterHovered(true)} onMouseLeave={() => setIsFilterHovered(false)} onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className={`h-12 flex items-center gap-2 px-4 rounded-2xl transition-all ${isFilterMenuOpen ? 'bg-[#004d4d] text-white shadow-lg' : 'bg-white text-slate-500 border border-gray-100 hover:text-[#004d4d] shadow-sm'} group`}>
                        ...
                        <AnimatePresence mode="popLayout">{(isFilterHovered || isFilterMenuOpen) && ( <motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap overflow-hidden ml-2">{filterStatus === 'all' ? 'Estado' : filterStatus}</motion.span> )}</AnimatePresence>
                                    ...
                                    <div className="relative">
                                        ...
                                        <AnimatePresence>
                                            {isDateMenuOpen && (
                                                <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="absolute top-full right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 z-50">
                                                    {[
                                                        ...
                                                        <button key={range.id} onClick={() => { setFilterDateRange(range.id); setIsDateMenuOpen(false); }} className={`w-full text-left px-4 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all ${filterDateRange === range.id ? 'bg-[#004d4d] text-white' : 'text-gray-500 hover:bg-gray-50'}`}>{range.label}</button>
                                                    ))}
                                                </motion.div>
                                            )}
                                        </AnimatePresence>
        ...
        <div className="px-4 pb-20">
            <div className="bg-white rounded-[3rem] border border-gray-100 shadow-xl overflow-hidden min-h-[500px]">
                <table className="w-full text-left">
                    <thead><tr className="bg-gray-50/50 border-b border-gray-100"><th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">Identidad Cliente</th><th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">Status</th><th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Frecuencia</th><th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">Inversión LTV</th><th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Acción</th></tr></thead>
                    <tbody className="divide-y divide-gray-50">
                        {loading ? ( <tr><td colSpan={5} className="py-40 text-center"><div className="flex flex-col items-center gap-4"><div className="w-10 h-10 border-4 border-[#004d4d]/10 border-t-[#00f2ff] rounded-full animate-spin"></div><p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Sincronizando Terminal Neural...</p></div></td></tr>
                        ) : filteredCustomers.map((c) => (
                            <tr key={c.id} onClick={() => setSelectedCustomer(c)} className="hover:bg-gray-50 transition-all group cursor-pointer"><td className="px-10 py-6"><div className="flex items-center gap-6"><div className="h-14 w-14 bg-gray-900 text-[#00f2ff] rounded-2xl flex items-center justify-center font-black text-xl shadow-lg group-hover:scale-110 transition-transform duration-500">{c.full_name.charAt(0)}</div><div><p className="text-sm font-black text-gray-900 tracking-tight">{c.full_name}</p><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{c.email}</p></div></div></td><td className="px-10 py-6"><span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${c.status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>{c.status}</span></td><td className="px-10 py-6 text-center"><div className="inline-flex items-center gap-2 px-3 py-1 bg-gray-50 rounded-lg border border-gray-100"><ShoppingBag size={12} className="text-[#004d4d]"/><span className="text-xs font-black text-[#004d4d]">{c.total_orders} Pedidos</span></div></td><td className="px-10 py-6"><span className="text-sm font-black text-gray-900 bg-[#00f2ff]/10 px-3 py-1 rounded-lg border border-[#00f2ff]/20">{formatCurrency(c.total_spent)}</span></td><td className="px-10 py-6 text-right"><button className="h-10 w-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-[#004d4d] group-hover:text-white transition-all shadow-sm"><ArrowUpRight size={18}/></button></td></tr>
                        ))}
                    </tbody>
                </table>
            </div>
