                    ...
                    <motion.div initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 50 }} className="bg-white w-full max-w-5xl max-h-[85vh] rounded-[3rem] md:rounded-[4rem] shadow-3xl overflow-hidden flex flex-col md:flex-row relative z-10 border border-white/20">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="p-6 bg-white/5 rounded-3xl border border-white/10 space-y-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div><p className="text-[8px] font-black text-white/40">Nueva comisión</p><p className="text-lg font-black tracking-tight">3.5%</p></div>
                                    </div>
                                    <p className="text-[9px] font-medium text-white/40 leading-tight">Aplicable a ventas futuras tras la activación.</p>
                                </div>
                            </div>
                            <div className="hidden md:block pt-8 border-t border-white/5"><p className="text-[8px] font-black text-white/30 tracking-[0.2em]">Acuerdo Bayup v2.0</p></div>
                        </div>
                        <div className="flex-1 flex flex-col bg-white overflow-hidden">
                            <div className="p-8 md:p-10 border-b border-gray-50 flex justify-between items-center bg-white/80 backdrop-blur-md shrink-0">
                                <div><h2 className="text-2xl font-black text-gray-900 tracking-tight italic leading-none">Confirmar escalado</h2><p className="text-gray-400 text-[10px] font-black tracking-widest mt-2 flex items-center gap-2"><ShieldCheck size={14} className="text-[#00f2ff]"/> Auditoría de desbloqueo</p></div>
                            ...
                            <div className="flex-1 overflow-y-auto p-8 md:p-10 space-y-8 custom-scrollbar">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {[
                                        ...
                                        <div key={i} className="p-5 bg-gray-50 rounded-2xl border border-transparent hover:border-gray-200 transition-all flex items-start gap-4">
                                            ...
                                            <div><p className="text-[11px] font-black text-gray-900 leading-none">{m.title}</p><p className="text-[10px] font-medium text-gray-400 mt-1.5 leading-tight">{m.desc}</p></div>
                                        </div>
                                    ))}
                                </div>
                                <div className="p-6 bg-amber-50/50 rounded-3xl border border-amber-100 flex items-start gap-5">
                                    ...
                                    <div className="space-y-1"><h4 className="text-xs font-black text-amber-900">Seguridad de datos</h4><p className="text-[10px] font-medium text-amber-800 leading-relaxed">Compartirás información de personal para liquidación. Datos protegidos bajo cifrado AES-256.</p></div>
                                </div>
                            </div>
                            <div className="p-8 md:p-10 border-t border-gray-50 bg-gray-50/30 flex flex-col sm:flex-row gap-4 shrink-0">
                                <button onClick={() => executeSwitch('pro')} disabled={isProcessing} className="flex-[2] py-5 bg-gray-900 text-white rounded-2xl font-black text-[10px] tracking-[0.2em] shadow-2xl hover:bg-black transition-all flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-50">
                                    {isProcessing ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={18} className="text-[#00f2ff]"/>}
                                    Aceptar & activar Plan Pro
                                </button>
                                <button onClick={() => setIsProModalOpen(false)} className="flex-1 py-5 bg-white border border-gray-100 text-gray-400 rounded-2xl font-black text-[10px] tracking-[0.2em] hover:text-gray-600 hover:border-gray-300 transition-all">Cancelar</button>
                            </div>
                        </div>
                    </motion.div>
                </div>
                ...
                <div className={`relative flex-1 flex flex-col p-10 rounded-[calc(4rem-2px)] border transition-all duration-500 h-full ${
                    ...
                    {isCurrent && (
                        <div className="absolute top-6 right-10 flex items-center gap-2 px-4 py-1.5 bg-[#00f2ff]/10 border border-[#00f2ff]/20 rounded-full">
                            ...
                            <span className="text-[10px] font-black tracking-widest text-[#004d4d]">Plan actual</span>
                        </div>
                    )}
                    ...
                    {isEmpresa && !isCurrent && (
                        <div className="absolute top-6 right-10 flex flex-col items-end">
                            ...
                            <span className="text-[8px] font-black bg-[#00f2ff] text-[#001a1a] px-2 py-0.5 rounded-full mt-2 tracking-tighter shadow-lg shadow-[#00f2ff]/20">Bayt AI enabled</span>
                        </div>
                    )}

                    <div className="mb-10">
                        <h3 className={`text-2xl font-black italic tracking-tighter ${isEmpresa ? 'text-white' : 'text-gray-900'}`}>{plan.name}</h3>
                    ...
                    <div className="mb-10 space-y-2">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-baseline gap-2">
                                ...
                                <span className={`text-[10px] font-black tracking-widest ${isEmpresa ? 'text-white/40' : 'text-gray-400'}`}>COP / Mes</span>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 pt-2">
                            <Zap size={14} className="text-[#00f2ff]" />
                            <p className={`text-[10px] font-black tracking-widest ${isEmpresa ? 'text-[#00f2ff]' : 'text-[#004d4d]'}`}>Comisión: {plan.commissionStr}</p>
                        </div>
                    </div>
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8 px-4">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-[10px] font-black tracking-[0.3em] text-[#004d4d]/60 italic">Motor de negocio v2.0</span>
                    </div>
                ...
                <div className="flex items-center gap-4 bg-white/60 backdrop-blur-xl p-4 rounded-3xl border border-white shadow-xl shadow-gray-100/50">
                    ...
                    <div><p className="text-[10px] font-black text-gray-400 tracking-widest">Próxima renovación</p><p className="text-sm font-black text-gray-900">01 de Marzo, 2026</p></div>
                </div>
            </div>
                ...
                <div className="bg-white/40 backdrop-blur-md p-8 rounded-[3rem] border border-white flex flex-col lg:flex-row items-center gap-8 shadow-xl shadow-gray-100/50 max-w-6xl mx-auto">
                    ...
                    <div className="w-full lg:w-[380px] bg-[#001a1a] rounded-[2.5rem] p-8 text-white space-y-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <h4 className="text-[10px] font-black text-[#00f2ff] tracking-[0.2em]">Resultado Bayup-IQ</h4>
                            {monthlySales >= breakEvenPoint ? (
                                <div className="mt-4 space-y-3">
                                    <div className="flex items-center gap-2 text-[#00f2ff] font-black italic text-lg tracking-tighter"> ¡Es hora de subir!</div>
                                ...
                                <div className="mt-4 space-y-3">
                                    <div className="flex items-center gap-2 text-blue-400 font-black italic text-lg tracking-tighter"><Check size={20}/> Plan Pro óptimo</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 px-4">
                <div className="lg:col-span-7 bg-white p-12 rounded-[4rem] border border-gray-100 shadow-xl space-y-10 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        ...
                        <div><h3 className="text-xl font-black text-gray-900 italic">Método de suscripción</h3><p className="text-[10px] font-black text-gray-400 tracking-widest">Donde se cargan tus servicios premium</p></div>
                    </div>
                    <div className="flex flex-col md:flex-row items-center justify-between p-8 bg-gray-50 rounded-[3rem] border border-transparent hover:border-[#004d4d]/20 transition-all relative z-10 gap-8">
                        <div className="flex items-center gap-6">
                            <div className="h-16 w-24 bg-gradient-to-br from-gray-800 to-black rounded-xl p-4 flex flex-col justify-between shadow-2xl">
                                <div className="flex justify-between items-start"><div className="h-4 w-6 bg-amber-400/20 rounded-sm"></div><div className="text-[8px] text-white/40 font-bold">VISA</div></div>
                                <div className="text-[10px] text-white font-mono tracking-widest">**** 4242</div>
                            </div>
                            <div><p className="text-lg font-black text-gray-900 italic">Visa Platinum</p><p className="text-xs font-bold text-gray-400 mt-1">Expira: 12 / 2026</p></div>
                        </div>
                        <div className="flex items-center gap-3">
                            <button className="h-12 px-6 bg-white border border-gray-200 text-gray-400 rounded-2xl font-black text-[9px] tracking-widest hover:text-[#004d4d] hover:border-[#004d4d] transition-all">Cambiar tarjeta</button>
