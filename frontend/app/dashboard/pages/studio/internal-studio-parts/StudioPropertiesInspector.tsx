    ...
    <div className="mb-4">
      <div onClick={() => setIsOpen(!isOpen)} className={cn("flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all border border-transparent", isOpen ? "bg-blue-50/50 border-blue-100" : "hover:bg-gray-50")}>
        ...
        <div className="flex items-center gap-2">
          {onRemove && <button onClick={(e) => { e.stopPropagation(); onRemove(); }} className="text-red-400 hover:text-red-600 p-1"><Trash2 size={12}/></button>}
...
export const DesignerInspector = () => {
  ...
  const addExtraElement = (type: "text" | "button" | "image") => {
    const newEl = {
      id: uuidv4(),
      type,
      ...(type === 'text' ? { content: "Nuevo Mensaje", size: 24, color: "#ffffff", variant: "solid", posX: 0, posY: 0, intensity: 100, font: "font-sans" } : 
         type === 'button' ? { text: "Ver Oferta", variant: "solid", bgColor: "#2563eb", textColor: "#ffffff", posX: 0, posY: 0, size: 14, intensity: 100, font: "font-black" } :
         { url: "", size: 150, posX: 0, posY: 0, radius: 12, intensity: 100, anim: "none" })
    };
  ...
  const renderHeroTextDesigner = (prefix: string, title: string) => {
    const get = (key: string) => {
      const fullKey = prefix ? `${prefix}${key.charAt(0).toUpperCase() + key.slice(1)}` : key;
      return element.props[fullKey];
    };
    const update = (key: string, val: any) => {
      const fullKey = prefix ? `${prefix}${key.charAt(0).toUpperCase() + key.slice(1)}` : key;
      handleChange(fullKey, val);
    };

    const variant = get("variant") || "solid";

    return (
      <ControlGroup title={title} icon={Type} defaultOpen={prefix === 'title'}>
        <div className="space-y-4">
          <textarea value={element.props[prefix] || ""} onChange={(e) => handleChange(prefix, e.target.value)} className="w-full p-3 border rounded-xl text-sm font-black bg-gray-50/30 uppercase italic" />
          
          <div className="space-y-2">
            <span className="text-[9px] font-black text-gray-400 uppercase">Temas de Diseño</span>
            <div className="grid grid-cols-3 gap-1 bg-gray-100 p-1 rounded-lg">
              {["solid", "outline", "3d", "brutalist", "aurora"].map(v => (
                <button key={v} onClick={() => update("variant", v)} className={cn("py-1.5 text-[8px] font-black uppercase rounded-md transition-all", (variant === v) ? "bg-white text-blue-600 shadow-sm" : "text-gray-400")}>{v}</button>
              ))}
            </div>
          </div>
          ...
          {variant === "aurora" && (
            <div className="p-3 bg-blue-50/50 rounded-xl border border-blue-100 space-y-2">
              <span className="text-[8px] font-black text-blue-600 uppercase">Colores Aurora</span>
              <div className="grid grid-cols-2 gap-2">
                <div className="flex items-center gap-2 p-1.5 bg-white rounded-lg border border-blue-100"><input type="color" value={get("aurora1") || "#00f2ff"} onChange={(e) => update("aurora1", e.target.value)} className="w-6 h-6 rounded-md p-0 cursor-pointer bg-transparent" /><span className="text-[8px] text-gray-400">Color 1</span></div>
                <div className="flex items-center gap-2 p-1.5 bg-white rounded-lg border border-blue-100"><input type="color" value={get("aurora2") || "#7000ff"} onChange={(e) => update("aurora2", e.target.value)} className="w-6 h-6 rounded-md p-0 cursor-pointer bg-transparent" /><span className="text-[8px] text-gray-400">Color 2</span></div>
              </div>
            </div>
          )}

          <div className="space-y-2">
            <span className="text-[9px] font-black text-gray-400 uppercase">Tipografía</span>
            <select value={get("font") || "font-sans"} onChange={(e) => update("font", e.target.value)} className="w-full p-2 border rounded-lg text-[10px] font-bold bg-white outline-none">
              <option value="font-black">Impact Black</option>
              ...
              <option value="font-extrabold">Ultra Bold</option>
            </select>
          </div>

          <div className="flex items-center gap-2 p-1.5 border rounded-xl bg-white"><input type="color" value={get("color") || "#ffffff"} onChange={(e) => update("color", e.target.value)} className="w-6 h-6 rounded-lg p-0 cursor-pointer" /><span className="text-[9px] text-gray-400 uppercase">Color Base</span></div>
          <FluidSlider label="Tamaño" value={get("size") || (prefix === 'title' ? 48 : 18)} min={10} max={120} onChange={(val:number) => update("size", val)} />
          <div className="py-2 bg-blue-50/30 rounded-xl px-2 border border-blue-100/50"><FluidSlider label="Intensidad Visual" value={get("intensity") || 100} min={0} max={200} onChange={(val:number) => update("intensity", val)} /></div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1 bg-gray-100 p-1 rounded-lg">
              {[{id:"none", l:"Normal", i:Wand2}, {id:"glow", l:"Resplandor", i:Zap}, {id:"neon", l:"Neon", i:Star}, {id:"fire", l:"Fuego", i:Flame}, {id:"float", l:"Flotante", i:Wind}].map(eff => (
                <button key={eff.id} onClick={() => update("effect", eff.id)} className={cn("flex items-center gap-2 p-2 text-[8px] font-black uppercase rounded-md transition-all", (get("effect") === eff.id || (!get("effect") && eff.id === "none")) ? "bg-white text-blue-600 shadow-sm" : "text-gray-400")}><eff.i size={10} /> {eff.l}</button>
              ))}
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3"><FluidSlider label="Posición X" value={get("posX") || 0} min={-500} max={500} onChange={(val:number) => update("posX", val)} /><FluidSlider label="Posición Y" value={get("posY") || 0} min={-200} max={200} onChange={(val:number) => update("posY", val)} /></div>
        </div>
      </ControlGroup>
    );
  };

  const renderHeroButtonDesigner = (prefix: string, title: string) => {
    const get = (key: string) => element.props[`${prefix}${key.charAt(0).toUpperCase() + key.slice(1)}`];
    const update = (key: string, val: any) => handleChange(`${prefix}${key.charAt(0).toUpperCase() + key.slice(1)}`, val);
    ...
    return (
      <ControlGroup title={title} icon={MousePointer2}>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-2">
            <div><span className="text-[9px] font-black text-gray-400 uppercase block mb-1">Etiqueta</span><input type="text" value={get("text") || ""} onChange={(e) => update("text", e.target.value)} className="w-full p-2 border rounded-lg text-xs font-bold" /></div>
            <div><span className="text-[9px] font-black text-gray-400 uppercase block mb-1">Vínculo</span><input type="text" value={get("url") || ""} onChange={(e) => update("url", e.target.value)} className="w-full p-2 border rounded-lg text-[10px] font-mono text-blue-600 placeholder:text-gray-300" placeholder="URL" /></div>
          </div>

          <div className="space-y-2">
            <span className="text-[9px] font-black text-gray-400 uppercase">Temas de Botón</span>
            <div className="grid grid-cols-3 gap-1 bg-gray-100 p-1 rounded-lg">
              {[{id:"solid", l:"Sólido"}, {id:"outline", l:"Borde"}, {id:"3d", l:"3D Pro"}, {id:"glass", l:"Glass"}, {id:"aurora", l:"Aurora"}, {id:"neon", l:"Neon"}, {id:"brutalist", l:"Brutal"}, {id:"retro", l:"Retro Glow"}].map(v => (
                <button key={v.id} onClick={() => update("variant", v.id)} className={cn("py-1.5 text-[7px] font-black uppercase rounded-md transition-all", (variant === v.id) ? "bg-white text-blue-600 shadow-sm" : "text-gray-400")}>{v.l}</button>
              ))}
            </div>
          </div>
          ...
          {variant === "neon" && (
            <div className="p-3 bg-blue-50/30 rounded-xl border border-blue-100/50">
              <FluidSlider label="Intensidad Neon" value={get("intensity") || 100} min={0} max={300} suffix="%" onChange={(val:number) => update("intensity", val)} />
            </div>
          )}

          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center gap-2 p-1.5 border rounded-lg bg-white h-[38px]"><input type="color" value={get("bgColor") || "#2563eb"} onChange={(e) => update("bgColor", e.target.value)} className="w-6 h-6 rounded-lg p-0 cursor-pointer" /><span className="text-[9px] text-gray-400 uppercase">Fondo</span></div>
            <div className="flex flex-col justify-end"><FluidSlider label="Tamaño" value={get("size") || 14} min={10} max={40} onChange={(val:number) => update("size", val)} /></div>
          </div>

          <FluidSlider label="Redondeo de Bordes" value={get("borderRadius") || 12} min={0} max={40} onChange={(val:number) => update("borderRadius", val)} />
        ...
        {activeTab === "content" && (
          <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div onClick={() => triggerUpload("logoUrl")} className="group border-2 border-dashed border-gray-200 rounded-2xl p-4 text-center cursor-pointer hover:border-blue-400 relative">
                      {element.props.logoUrl ? (
                        <div className="relative h-12 mx-auto"><img src={element.props.logoUrl} className="h-full object-contain" /><button onClick={(e) => { e.stopPropagation(); handleChange("logoUrl", null); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow-md hover:bg-red-600 transition-colors"><X size={10}/></button></div>
                      ) : <p className="text-[10px] font-bold text-gray-400 uppercase">SUBIR LOGO</p>}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3"><select value={element.props.logoFont || "font-black"} onChange={(e) => handleChange("logoFont", e.target.value)} className="w-full p-2 border rounded-lg text-[10px] font-bold"><option value="font-black">Black</option><option value="font-sans">Modern</option></select><div className="flex items-center gap-2 p-1.5 border rounded-lg"><input type="color" value={element.props.logoColor || "#2563eb"} onChange={(e) => handleChange("logoColor", e.target.value)} className="w-6 h-6 rounded-lg cursor-pointer" /><span className="text-[9px] text-gray-400 uppercase">Color</span></div></div>
                  </div>
                </ControlGroup>
            ...
            {element.type === "hero-banner" && (
              <>
                {renderHeroTextDesigner("title", "Título de Impacto")}
                {renderHeroTextDesigner("subtitle", "Descripción")}
                
                {renderHeroButtonDesigner("primaryBtn", "Botón Primario")}
                {renderHeroButtonDesigner("secondaryBtn", "Botón Secundario")}

                <ControlGroup title="Imagen de Complemento" icon={ImageIcon}>
                  <div className="space-y-4">
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                      {[{id:"image", l:"Imagen"}, {id:"video", l:"Video"}].map(t => (
                        <button key={t.id} onClick={() => handleChange("floatType", t.id)} className={cn("flex-1 py-1.5 text-[9px] font-black uppercase rounded-md transition-all", (element.props.floatType === t.id || (!element.props.floatType && t.id === "image")) ? "bg-white shadow-sm text-blue-600" : "text-gray-400")}>{t.l}</button>
                      ))}
                    </div>
                    {element.props.floatType === "video" ? (
                      <div className="space-y-4">
                        <div onClick={() => triggerUpload("floatUrl")} className="group border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-blue-400 cursor-pointer relative transition-all">
                          {element.props.floatUrl ? (
                            <div className="relative aspect-video rounded-lg overflow-hidden border border-gray-100 bg-black flex items-center justify-center">
                              <Play size={20} className="text-white opacity-50" />
                              <button onClick={(e) => { e.stopPropagation(); handleChange("floatUrl", null); }} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 shadow-lg hover:bg-red-600 transition-colors"><X size={10} /></button>
                            </div>
                          ) : <div className="py-2"><Play size={20} className="mx-auto text-gray-300 mb-1" /><p className="text-[8px] font-bold text-gray-400 uppercase">SUBIR VIDEO (MP4, WEBM)</p></div>}
                        </div>
                        <input type="text" value={element.props.floatUrl || ""} onChange={(e) => handleChange("floatUrl", e.target.value)} className="w-full p-2 border rounded-lg text-[10px] font-mono text-blue-600 bg-gray-50/30" placeholder="O pega URL de video..." />
                      </div>
                    ) : (
                      <div onClick={() => triggerUpload("floatUrl")} className="group border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-blue-400 cursor-pointer relative">
                        {element.props.floatUrl ? (
                          <div className="relative aspect-square w-20 mx-auto rounded-lg overflow-hidden border">
                            <img src={element.props.floatUrl} className="w-full h-full object-cover" />
                            <button onClick={(e) => { e.stopPropagation(); handleChange("floatUrl", null); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow-md hover:bg-red-600 transition-colors"><X size={10}/></button>
                          </div>
                    ...
                    <div className="space-y-2">
                      <span className="text-[9px] font-black text-gray-400 uppercase">Efectos de Complemento</span>
                      <div className="grid grid-cols-3 gap-1 bg-gray-100 p-1 rounded-lg">
                        {[{id:"none", l:"Normal", i:Wand2}, {id:"float", l:"Flotar", i:Wind}, {id:"zoom", l:"Zoom", i:Maximize}, {id:"blink", l:"Parpadeo", i:Zap}, {id:"rotate", l:"Rotar", i:RotateCw}, {id:"pulse", l:"Pulso", i:Star}].map(eff => (
                          <button key={eff.id} onClick={() => handleChange("floatAnim", eff.id)} className={cn("flex flex-col items-center gap-1 p-2 text-[6px] font-black uppercase rounded-md transition-all", (element.props.floatAnim === eff.id || (!element.props.floatAnim && eff.id === "none")) ? "bg-white text-blue-600 shadow-sm" : "text-gray-400")}><eff.i size={10} /> {eff.l}</button>
                        ))}
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3"><FluidSlider label="Escala" value={element.props.floatSize || 150} min={50} max={400} onChange={(v:number) => handleChange("floatSize", v)} /><FluidSlider label="Puntas" value={element.props.floatRadius || 12} min={0} max={100} onChange={(v:number) => handleChange("floatRadius", v)} /></div>
                    <div className="grid grid-cols-2 gap-3"><FluidSlider label="Posición X" value={element.props.floatPosX || 0} min={-600} max={600} onChange={(v:number) => handleChange("floatPosX", v)} /><FluidSlider label="Posición Y" value={element.props.floatPosY || 0} min={-300} max={300} onChange={(v:number) => handleChange("floatPosY", v)} /></div>
                  </div>
                </ControlGroup>
                ...
                <div className="relative mt-8">
                  ...
                  <AnimatePresence>{showAddMenu && ( <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-full left-0 w-full bg-white border border-gray-100 shadow-2xl rounded-2xl p-2 z-50 mb-2 grid grid-cols-2 gap-2"> {[{id:'text', l:'Texto', i:Type}, {id:'button', l:'Botón', i:MousePointer2}].map(opt => ( <button key={opt.id} onClick={() => { const newEl = { id: uuidv4(), type: opt.id, content: "Nuevo", text: "Nuevo", size: 24, color: "#ffffff", bgColor: "#2563eb", posX: 0, posY: 0 }; handleChange("extraElements", [...(element.props.extraElements || []), newEl]); setShowAddMenu(false); }} className="flex flex-col items-center gap-2 p-3 hover:bg-blue-50 rounded-xl transition-all"><div className="p-2 bg-blue-500 text-white rounded-lg shadow-sm"><opt.i size={16}/></div><span className="text-[9px] font-black uppercase text-gray-600">{opt.l}</span></button> ))} </motion.div> )}</AnimatePresence>
                </div>
              </>
            )}
          </div>
                  ...
                  {element.props.bgType === "video" ? (
                    <div className="space-y-4">
                      <div onClick={() => triggerUpload("videoUrl")} className="group border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-blue-400 cursor-pointer relative transition-all">
                        {element.props.videoUrl ? <div className="relative aspect-video rounded-lg overflow-hidden border border-gray-100 bg-black flex items-center justify-center"><Play size={24} className="text-white opacity-50" /><button onClick={(e) => { e.stopPropagation(); handleChange("videoUrl", null); }} className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 shadow-md hover:bg-red-600 transition-colors"><X size={12} /></button></div> : <div className="py-2"><Play size={24} className="mx-auto text-gray-300 mb-1" /><p className="text-[9px] font-bold text-gray-400 uppercase">SUBIR VIDEO (MP4, WEBM)</p></div>}
                      </div>
