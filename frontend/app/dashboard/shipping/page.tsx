...
import { 
  Search, Filter, ChevronRight, ChevronDown, X, CheckCircle2, Truck, Package, 
  AlertCircle, ArrowUpDown, Download, RefreshCcw, Globe, Smartphone, CreditCard, 
  Calendar as CalendarIcon, MapPin, Map, Info, Printer, ExternalLink, User, Zap, 
  TrendingUp, TrendingDown, Clock, RotateCcw, Box, Activity
} from 'lucide-react';
            ...
            <div className={`relative h-full bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm transition-all duration-300 group-hover:shadow-2xl overflow-hidden`}>
                ...
                <div className="relative z-10 flex flex-col h-full justify-between" style={{ transform: "translateZ(30px)" }}>
                    <div className="flex items-start justify-between">
                        ...
                        {data.trend && (
                             <div className={`flex items-center gap-1 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider ${data.trendUp ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>
                                {data.trendUp ? <TrendingUp size={12}/> : <TrendingDown size={12}/>} {data.trend}
                             </div>
                        )}
                    </div>
                    <div className="mt-4">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">{data.title}</p>
                        <h3 className="text-4xl font-black text-slate-900 tracking-tighter italic">{data.value}</h3>
                    </div>
                </div>
            </div>
        </motion.div>
...
const MetricModal = ({ metric, onClose }: { metric: MetricData | null, onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden border border-white">
...
export default function ShippingPage() {
  ...
  useEffect(() => {
    ...
    if (savedShipments.length > 0) {
        setShipments(prev => {
            ...
            return combined.filter((v, i, a) => a.findIndex(t => t.order_id === v.order_id) === i);
        });
    }
  }, []);
  ...
  // --- Handlers ---
  const getTrackingUrl = (c?: string, code?: string) => {
    if (!c || !code) return "#";
    if (c === 'Coordinadora') return `https://www.coordinadora.com/portafolio-de-servicios/servicios-en-linea/rastreo-de-guias/?guia=${code}`;
    if (c === 'Servientrega') return `https://www.servientrega.com/wps/portal/Colombia/transacciones-personas/rastreo-envios?id=${code}`;
    return "#";
  };
  ...
  const handleExportExcel = () => {
    const tableHtml = `<html><body><table><tr><td>ID ENVÍO</td><td>CLIENTE</td><td>GUÍA</td></tr>${shipments.map(s => `<tr><td>${s.id}</td><td>${s.customer.name}</td><td>${s.tracking_number}</td></tr>`).join('')}</table></body></html>`;
    const blob = new Blob([tableHtml], { type: 'application/vnd.ms-excel' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a"); link.href = url; link.download = "Envios.xls"; link.click();
  };

  const guideContent = {
    all: { title: 'Vista 360', icon: <Package size={20}/>, color: 'text-blue-500', howItWorks: 'Visión global.', example: 'Auditoría rápida.', tip: 'Usa filtros.' },
    label_generated: { title: 'Guía', icon: <Printer size={20}/>, color: 'text-slate-500', howItWorks: 'Etiqueta lista.', example: 'Pendiente recolección.', tip: 'Empaque premium.' },
    in_transit: { title: 'Tránsito', icon: <Truck size={20}/>, color: 'text-blue-600', howItWorks: 'En viaje.', example: 'Entre ciudades.', tip: 'Monitorea 48h.' },
    out_for_delivery: { title: 'Reparto', icon: <Activity size={20}/>, color: 'text-cyan-600', howItWorks: 'Última milla.', example: 'Mensajero local.', tip: 'Avisa al cliente.' },
    delivered: { title: 'Entregado', icon: <CheckCircle2 size={20}/>, color: 'text-emerald-600', howItWorks: 'Éxito total.', example: 'Producto recibido.', tip: 'Pide reseña.' },
    incident: { title: 'Incidencia', icon: <AlertCircle size={20}/>, color: 'text-rose-600', howItWorks: 'Novedad crítica.', example: 'Dirección errada.', tip: 'Llama rápido.' }
  };
        ...
        <div className="flex flex-col items-center gap-6">
            ...
            <div className="w-full flex justify-between items-center bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
                 ...
                 <div className="flex items-center gap-1">
                     ...
                     <motion.button layout onMouseEnter={() => setIsExportHovered(true)} onMouseLeave={() => setIsExportHovered(false)} onClick={handleExportExcel} className="h-12 flex items-center gap-2 px-4 rounded-2xl bg-white border border-slate-100 text-slate-500">
            ...
            <>
                ...
                <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed inset-y-0 right-0 w-full max-w-xl bg-white shadow-2xl z-[210] border-l flex flex-col">
                    ...
                    <div className="p-8 border-t bg-white grid grid-cols-2 gap-4">
                        ...
                        <button onClick={() => window.open(getTrackingUrl(selectedShipment.carrier, selectedShipment.tracking_number))} className="py-4 bg-[#004D4D] text-white rounded-2xl font-black text-[10px] uppercase shadow-lg">Rastrear API</button>
                    </div>
                </motion.div>
            </>
        )}
            ...
            <div className="fixed inset-0 z-[600] flex items-center justify-center p-4">
                ...
                <motion.div initial={{ scale: 0.95, y: 30 }} animate={{ scale: 1, y: 0 }} className="relative bg-white w-full max-w-4xl h-[70vh] rounded-[3rem] shadow-2xl flex overflow-hidden">
                    ...
                    <div className="flex-1 p-10 space-y-10">
                        <h2 className="text-3xl font-black italic uppercase">{guideContent[activeGuideGuideTab as keyof typeof guideContent]?.title || 'Guía'}</h2>
