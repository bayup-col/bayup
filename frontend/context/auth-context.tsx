  ...
  const syncProfile = useCallback(async (authToken?: string | null) => {
    ...
    try {
      ...
      if (e?.message === 'Could not validate credentials') {
        // Solo cerrar sesión si esta petición sigue vigente.
        // Si login() ya reemplazó el token (p.ej. tras Google OAuth) mientras
        // esta petición estaba en vuelo, profileEpochRef ya avanzó: ignorar.
        if (profileEpochRef.current !== epoch) return;
        ...
        router.push(onSuperAdmin ? '/bayup-family' : '/login');
      }
